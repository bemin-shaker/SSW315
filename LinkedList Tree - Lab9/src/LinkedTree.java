...
public class LinkedTree {
    ...
    public static int sumKeysTree(Node root) {
    	if (root == null)  
            return 0;  
        return (root.key + sumKeysTree(root.left) +  
        		sumKeysTree(root.right));  
    }
    
    /*
     * Iteratively computes the sum of the keys along the path from 
     * the root to the node containing the specified key (if any).
     * If the specified key is not in the tree, the method returns 
     * the sum of the keys along the path used to determine that the 
     * key is missing.
     */
